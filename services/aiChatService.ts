...
function buildSystemInstruction({
  ...
  const spendingLines = spendingCategories
    .map(
      (category) => `- ${category.label}: ${formatCurrency(category.amount)}`,
    )
    .join("\n");
  const incomeLines = cashFlow.incomeSources
    .map((source) => `- ${source.label}: ${formatCurrency(source.amount)}`)
    .join("\n");
  const assetLines = balanceSheet.assets
    .map((asset) => `- ${asset.label}: ${formatCurrency(asset.amount)}`)
    .join("\n");
  const liabilityLines = balanceSheet.liabilities
    .map(
      (liability) =>
        `- ${liability.label}: ${formatCurrency(liability.amount)}`,
    )
    .join("\n");
 ...
 */
export async function getFinancialChatReply(
  question: string,
