...
function IncomeOptionCard({ option, isSelected, isAnySelected, onSelect }: IncomeOptionCardProps) {
  ...
  useEffect(() => {
    dim.value = withSpring(isAnySelected && !isSelected ? 0.4 : 1, springs.default);
  ...
  return (
    <Animated.View style={cardDimStyle}>
      ...
      >
        <Box className={`mr-4 h-12 w-12 items-center justify-center rounded-md ${isSelected ? 'bg-primary' : 'bg-primary/16'}`}>
          <Ionicons name={option.icon} size={24} color={isSelected ? '#FFFFFF' : colors.primary} />
        </Box>
        <Box className="flex-1">
          <Text className="font-display-semibold text-[17px] text-foreground">{option.label}</Text>
          <Text className="mt-0.5 font-body text-[13px] text-muted-foreground">{option.description}</Text>
        </Box>
